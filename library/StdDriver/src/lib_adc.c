...
{
    ADC->VREFCON.VREF_EN = ENUM_ENABLE;                 //内部VREFP使能
    ADC->VREFCON.IREF_EN = ENUM_ENABLE;                 //IREF使能位
...
ErrorStatus ADC_ACPConfig(ADC_ACP_TypeDef *ADC_ACP_InitStruct)
{
    ...
    if (ADC_ACP_InitStruct->ACP_EN == ENUM_ENABLE)
    {
