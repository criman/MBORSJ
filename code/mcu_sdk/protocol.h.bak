...
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
//#define PRODUCT_KEY "pwuwq1xm6vx9ebkn"    //开发平台创建产品后生成的16位字符产品唯一标识
#define PRODUCT_KEY "gay9pzalzwnzr2bi"    //开发平台创建产品后生成的16位字符产品唯一标识
...
#define WIFI_UART_RECV_BUF_LMT          (16+128)              //串口数据接收缓存区大小,如MCU的RAM不够,可缩小
#define WIFI_DATA_PROCESS_LMT           (24+128)              //串口数据处理缓存区大小,根据用户DP数据大小量定,必须大于24
#else
...
#define WIFIR_UART_SEND_BUF_LMT         (48+128)              //根据用户DP数据大小量定,必须大于48
/******************************************************************************
                        4:定义模块工作方式
模块自处理:
...
//开关(可下发可上报)
//备注:
#define DPID_SWITCH_ON 1
//定时(可下发可上报)
//备注:第一位普通定时关机；第二位普通定时开机；
#define DPID_TIMER_FUNC 4
//模式(可下发可上报)
//备注:0智能；1制热；2除湿；3制冷；4送风
#define DPID_SET_MODE 5
//摄氏温度(可下发可上报)
//备注:
#define DPID_SET_TEMPER 6
//风速(可下发可上报)
//备注:0自动，1低速，2中速，3高速，
#define DPID_WIND_SPEED 8
//(可下发可上报)
//备注:1:华氏 0:摄氏
#define DPID_C_F_CHANGE 10
//定时关机(可下发可上报)
//备注:
#define DPID_OFF_TIMER_MIN 13
//定时开机(可下发可上报)
//备注:
#define DPID_ON_TIMER_MIN 14
//故障(只上报)
//备注:Lt:低温除霜保护；
//PF:感温系统故障；
//Ft:安全水箱水满；
...
//EA:室外环境温度传感器故障
//E7:室外排气温度传感器故障
#define DPID_FAULT 15
//摆风(可下发可上报)
//备注:
#define DPID_WIND 16
//睡眠(可下发可上报)
//备注:0:关 1:开
#define DPID_SLEEP_FUNC 17
//华氏温度(可下发可上报)
//备注:
#define DPID_F_TEMPER 18
//强力(可下发可上报)
//备注:
#define DPID_FORCE 19
