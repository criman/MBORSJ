...
{
  {DPID_SWITCH_ON, DP_TYPE_BOOL},
  {DPID_TIMER_FUNC, DP_TYPE_BITMAP},
  {DPID_SET_MODE, DP_TYPE_ENUM},
  {DPID_SET_TEMPER, DP_TYPE_VALUE},
  {DPID_WIND_SPEED, DP_TYPE_ENUM},
  {DPID_C_F_CHANGE, DP_TYPE_BOOL},
  {DPID_OFF_TIMER_MIN, DP_TYPE_VALUE},
  {DPID_ON_TIMER_MIN, DP_TYPE_VALUE},
  {DPID_FAULT, DP_TYPE_BITMAP},
  {DPID_WIND, DP_TYPE_BOOL},
  {DPID_SLEEP_FUNC, DP_TYPE_BOOL},
  {DPID_F_TEMPER, DP_TYPE_VALUE},
  {DPID_FORCE, DP_TYPE_BOOL},
};
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_bool_update(DPID_SWITCH_ON, WiFi.Api.f_OnOff); 	  			//BOOL型数据上报;
	
    mcu_dp_fault_update(DPID_TIMER_FUNC, WiFi.u8_TimerOnOff);  			//故障型数据上报;
	
    mcu_dp_enum_update(DPID_SET_MODE, WiFi.Enum_WSysMode); 	  			//枚举型数据上报;
	
    mcu_dp_value_update(DPID_SET_TEMPER, WiFi.u8_TempCSet);   			//VALUE型数据上报;
		
    mcu_dp_enum_update(DPID_WIND_SPEED, WiFi.Enum_WFanMode);  			//枚举型数据上报;
		
    mcu_dp_bool_update(DPID_C_F_CHANGE, WiFi.Api.f_TempUnit); 			//BOOL型数据上报;
	
    mcu_dp_value_update(DPID_OFF_TIMER_MIN, WiFi.u16_TimerOffMinute); 	//VALUE型数据上报;
    
	mcu_dp_value_update(DPID_ON_TIMER_MIN, WiFi.u16_TimerOnMinute); 	//VALUE型数据上报;
		
    mcu_dp_fault_update(DPID_FAULT, WiFi.u8_Fault); 					//故障型数据上报;
    	
	mcu_dp_bool_update(DPID_WIND, WiFi.Api.f_Sweep); 					//BOOL型数据上报;
    	
	mcu_dp_bool_update(DPID_SLEEP_FUNC, WiFi.Api.f_Sleep); 				//BOOL型数据上报;
    	
	mcu_dp_value_update(DPID_F_TEMPER, WiFi.u8_TempFSet); 				//VALUE型数据上报;
    
	mcu_dp_bool_update(DPID_FORCE, WiFi.Api.f_Force); 					//BOOL型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_switch_on_handle
功能描述 : 针对DPID_SWITCH_ON的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_switch_on_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    ...
    switch_on = mcu_get_dp_download_bool(value,length);
//    if(switch_on == 0) {
//        //bool off
//    }else {
//        //bool on
//    }
	...
	App_uartapi_onoff(switch_on);
	
  
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_SWITCH_ON,switch_on);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_timer_func_handle
功能描述 : 针对DPID_TIMER_FUNC的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_timer_func_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BITMAP
    unsigned char ret;
	unsigned char timeronoff;
    /*
    //FAULT type data processing
    
    */
    
	timeronoff = mcu_get_dp_download_value(value,length);
	
	App_uartapi_timeronoff(timeronoff);
	
    //There should be a report after processing the DP
    ret = mcu_dp_fault_update(DPID_TIMER_FUNC,(unsigned long)value);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_set_mode_handle
功能描述 : 针对DPID_SET_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_set_mode_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char set_mode;
    
    set_mode = mcu_get_dp_download_enum(value,length);
//    switch(set_mode) {
//        case 0:
//        break;
//        
...
//    }
	...
	App_uartapi_wsysmode(set_mode);

    
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_SET_MODE, set_mode);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_set_temper_handle
功能描述 : 针对DPID_SET_TEMPER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_set_temper_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long set_temper;
    
    set_temper = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    
    */
	
	App_uartapi_tempcset(set_temper);
	
    
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_SET_TEMPER,set_temper);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_wind_speed_handle
功能描述 : 针对DPID_WIND_SPEED的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_wind_speed_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char wind_speed;
    
    wind_speed = mcu_get_dp_download_enum(value,length);
//    switch(wind_speed) {
//        case 0:
//        break;
//        
...
//    }
	...
	App_uartapi_fanmode(wind_speed);
	
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_WIND_SPEED, wind_speed);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_c_f_change_handle
功能描述 : 针对DPID_C_F_CHANGE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_c_f_change_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char c_f_change;
    
    c_f_change = mcu_get_dp_download_bool(value,length);
//    if(c_f_change == 0) {
//        //bool off
//    }else {
//        //bool on
//    }
	...
	App_uartapi_tempunit(c_f_change);
  
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_C_F_CHANGE,c_f_change);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_off_timer_min_handle
功能描述 : 针对DPID_OFF_TIMER_MIN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_off_timer_min_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long off_timer_min;
    
    off_timer_min = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    
    */
	
	App_uartapi_timeroff(off_timer_min);
    
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_OFF_TIMER_MIN,off_timer_min);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_on_timer_min_handle
功能描述 : 针对DPID_ON_TIMER_MIN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_on_timer_min_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long on_timer_min;
    
    on_timer_min = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    
    */
    
	App_uartapi_timeron(on_timer_min);
	
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_ON_TIMER_MIN,on_timer_min);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_wind_handle
功能描述 : 针对DPID_WIND的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_wind_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    ...
    wind = mcu_get_dp_download_bool(value,length);
//    if(wind == 0) {
//        //bool off
//    }else {
//        //bool on
//    }
	...
	App_uartapi_sweep(wind);
	
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_WIND,wind);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_sleep_func_handle
功能描述 : 针对DPID_SLEEP_FUNC的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_sleep_func_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char sleep_func;
    
    sleep_func = mcu_get_dp_download_bool(value,length);
//    if(sleep_func == 0) {
//        //bool off
//    }else {
//        //bool on
//    }
	...
	App_uartapi_sleep(sleep_func);
  
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_SLEEP_FUNC,sleep_func);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_f_temper_handle
功能描述 : 针对DPID_F_TEMPER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_f_temper_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long f_temper;
    
    f_temper = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    
    */
	
	App_uartapi_tempfset(f_temper);
    
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_F_TEMPER,f_temper);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_force_handle
功能描述 : 针对DPID_FORCE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_force_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    ...
    force = mcu_get_dp_download_bool(value,length);
//    if(force == 0) {
//        //bool off
//    }else {
//        //bool on
//    }
	...
	App_uartapi_force(force);
  
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_FORCE,force);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_SWITCH_ON:
            //开关处理函数
            ret = dp_download_switch_on_handle(value,length);
        break;
        case DPID_TIMER_FUNC:
            //定时处理函数
            ret = dp_download_timer_func_handle(value,length);
        break;
        case DPID_SET_MODE:
            //模式处理函数
            ret = dp_download_set_mode_handle(value,length);
        break;
        case DPID_SET_TEMPER:
            //摄氏温度处理函数
            ret = dp_download_set_temper_handle(value,length);
        break;
        case DPID_WIND_SPEED:
            //风速处理函数
            ret = dp_download_wind_speed_handle(value,length);
        break;
        case DPID_C_F_CHANGE:
            //处理函数
            ret = dp_download_c_f_change_handle(value,length);
        break;
        case DPID_OFF_TIMER_MIN:
            //定时关机处理函数
            ret = dp_download_off_timer_min_handle(value,length);
        break;
        case DPID_ON_TIMER_MIN:
            //定时开机处理函数
            ret = dp_download_on_timer_min_handle(value,length);
        break;
        case DPID_WIND:
            //摆风处理函数
            ret = dp_download_wind_handle(value,length);
        break;
        case DPID_SLEEP_FUNC:
            //睡眠处理函数
            ret = dp_download_sleep_func_handle(value,length);
        break;
        case DPID_F_TEMPER:
            //华氏温度处理函数
            ret = dp_download_f_temper_handle(value,length);
        break;
        case DPID_FORCE:
            //强力处理函数
            ret = dp_download_force_handle(value,length);
        break;
